...
class icarus_spider(CrawlSpider):#CrawlSpider类继承自Spider类，可以通过定义rules属性实现递归抓取
    ...
    start_urls = ["http://icarus.silversky.moe:666/category/illustration/super-res-wallpaper/page/1"]#起始url
 
    #定义rules属性
    rules = (
        Rule(SgmlLinkExtractor(allow=(r'\/category\/illustration\/super-res-wallpaper\/page\/\d')),#使用正则表达式匹配符合要求的网页
             callback=None,#回调函数
